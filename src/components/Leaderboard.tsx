...
export const Leaderboard = () => {
  ...
  const { data: userAddresses } = useContractRead({
  ...
  // Ambil data user untuk setiap address
  const { data: usersData } = useContractReads({
    contracts: Array.isArray(userAddresses)
      ...
      : [],
    watch: true,
    enabled: Array.isArray(userAddresses) && userAddresses.length > 0,
  });

  useEffect(() => {
    if (Array.isArray(usersData)) {
      // Map dan urutkan leaderboard berdasarkan totalXP
      const leaderboardArr = usersData
        .map((result: any, idx: number) => {
          ...
          return {
            ...
            displayName: user.username, // Ganti jika ada displayName
            avatar: `https://api.dicebear.com/7.x/avataaars/svg?seed=${user.username}`,
            totalXP: Number(user.totalXP),
            level: Number(user.level),
            questsCompleted: Number(user.questsCompleted),
          };
        })
        .filter(Boolean)
